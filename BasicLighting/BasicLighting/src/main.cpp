...
//
//	cv::Mat tmp = cv::imread("container.jpg");
//	cv::imshow("1", tmp);
//	width = tmp.cols;
//	height = tmp.rows;
//	nrChannels = tmp.channels();
//
//	//unsigned char *data = stbi_load("container.jpg", &width, &height, &nrChannels, 0);
//	if (tmp.data)
//	{
...
//		std::cout << "Failed to load texture" << std::endl;
//	}
//	//stbi_image_free(tmp.data);   // 释放图片数据
//
//	// 纹理2
...
//		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_BGRA, GL_UNSIGNED_BYTE, tmp.data);  // opencv中PNG是BGRA存储
//		glGenerateMipmap(GL_TEXTURE_2D);
//	}
//	else
//	{
...
int main()
{
	...
	Shader lampShader("src/1.lamp.vs", "src/light.fs");      // 实例化光源对象
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		lightingShader.setVec3("objectColor", lightColor);           // 物体颜色
		lightingShader.setVec3("lightColor",  1.0f, 0.5f, 1.0f);     // 光照颜色
