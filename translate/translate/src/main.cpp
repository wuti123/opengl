...
int main()
{
	...
	// 创建着色器类对象
	Shader ourShader("src/translate.vs", "src/translate.fs");  // 从工程文件目录的位置开始检索
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 创建4x4单位矩阵，然后经平移变换，经旋转变换，经缩放变换，最后与顶点位置向量相乘
		// （将这几个矩阵和向量写出来，可以发现实际上是先缩放矩阵与向量相乘，相乘结果再与旋转矩阵相乘，结果再与平移矩阵相乘）
		// 先进行缩放操作，然后是旋转，最后才是位移
		glm::mat4 trans = glm::mat4(1.0f);  // 定义一个4x4单位矩阵
		// 将矩形平移到屏幕右下角
		...
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);   // 矩形由两个三角形组成，故是6个顶点,画第一个箱子
		...
		/*glDrawArrays(GL_TRIANGLES, 0, 3);*/
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);   // 矩形由两个三角形组成，故是6个顶点，画第二个箱子
