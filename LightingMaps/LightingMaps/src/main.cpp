...
int main()
{
	...
	vector<struct Vertex> new_vertices;
	struct Vertex tmp_vertice;
	for (int i = 0; i < sizeof(vertices) / (8 * sizeof(float)); ++i)
	{
		for (int j = 0; j < 8; ++j)
		{
			switch (j)
			{
			case 0:
				tmp_vertice.Position.x = vertices[8 * i + j];
				break;
			case 1:
				tmp_vertice.Position.y = vertices[8 * i + j];
				break;
			case 2:
				tmp_vertice.Position.z = vertices[8 * i + j];
				break;
			case 3:
				tmp_vertice.Normal.x = vertices[8 * i + j];
				break;
			case 4:
				tmp_vertice.Normal.y = vertices[8 * i + j];
				break;
			case 5:
				tmp_vertice.Normal.z = vertices[8 * i + j];
				break;
			case 6:
				tmp_vertice.TexCoords.x = vertices[8 * i + j];
				break;
			case 7:
				tmp_vertice.TexCoords.y = vertices[8 * i + j];
				break;
			default:
				break;
			}
		}
		new_vertices.push_back(tmp_vertice);
	}
	//cout << "size: " << new_vertices.size() << endl;
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(struct Vertex) * new_vertices.size(), &new_vertices[0], GL_STATIC_DRAW);
	...
	lightingShader.use();
	lightingShader.setInt("material.diffuse", diffuseMap);  // 将纹理单元赋值给material.diffuse这个uniform采样器

	// 绑定漫反射贴图
	//glActiveTexture(GL_TEXTURE1);   // 纹理单元编号为diffuseMap == 1
	glBindTexture(GL_TEXTURE_2D, diffuseMap);
	cout << "diffuseMap: " << diffuseMap << endl;
